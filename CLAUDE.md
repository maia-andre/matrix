...
Os mostradores (`modelo`, `agencia`, `modelo_do_outro`, `phi`, `relato`), todos em `[0,1]`, medem se uma faculdade **carrega o comportamento** (famílias: ablação e calibração) — função, nunca experiência. Aparecem no HUD e no CSV (`--log`). Colunas do CSV: `seed, tick, pop, energia_media, comida_total`, média e desvio dos 4 traços (`*_m`, `*_sd`), os 5 mostradores, e `hon_f`/`blef_f` (fração da população por estratégia de sinalização — o relato causal, Fase 4). Ao adicionar/renomear coluna, manter a reprodutibilidade e atualizar o README (e o notebook, que lê colunas por nome). (`modelo_do_outro` era `automodelo` — renomeado na nota 04: mede o outro, não o self. `phi` foi redefinida na nota 05. `relato` foi pré-registrado no ROADMAP §2.0 **antes** de ser construído — nota 06; na v1 ele é medido mas não consumido: epifenomenal por construção.)
